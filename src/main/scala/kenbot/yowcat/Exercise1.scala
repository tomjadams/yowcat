...
object Exercise1 {
  ...
  object ClassesSubtypesCategory extends Cat {
    ...
    //def arrows: Stream[Arr] = setOfClasses.flatMap { a => setOfClasses.withFilter { b => a.isSubTypeOf(b) }.map { b => (a, b) } }
    def arrows: Stream[Arr] = for {
      a <- setOfClasses
      b <- setOfClasses if a.isSubTypeOf(b)
    } yield (a, b)
